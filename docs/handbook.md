...
| Profile | Gender, birth year, **body weight as a dated series**. Names what is still missing rather than failing silently. ⚠️ **ITS THREE `WHY` PARAGRAPHS ARE BEHIND A "?" SINCE 2026-09-08** — a dot beside each field label, which is where Hevy puts its one explanation too. 🆕 **`.help-line.by-label`** exists because the app-wide rule right-aligns the dot, which is right above a body map and wrong beside a two-word label; this screen stacks both shapes and the mismatch showed |
...
- ⚠️ **WHERE THE DOT SITS IS PART OF THE RULE.** `.help-line` right-aligns it, which is right when
  the ? explains a block the width of the screen. Beside a two-word field label it strands the dot
  300px from the word it answers for, so `.help-line.by-label` keeps it against the words. Profile
  stacks both shapes and the mismatch was visible on the screen.
