...
16. **🚨 HOW TO RUN THE SHIPPED FIREBASE MODULE AGAINST THE REAL PROJECT — `tools/live-check.mjs`,
    2026-09-17.** The emulator answers what the rules **in this working tree** do; a test double
    answers arithmetic. Neither can tell you what the **deployed** rules do, and neither can bill
    anybody — so the two things this app cannot otherwise check are a rules deploy that did not land
    (silent: `republish()` is fire-and-forget) and the read pattern's cost (`docs/running-costs.html`).

    ```
    npm i --no-save firebase@10.12.2         # into a scratch dir; see --sdk
    node tools/live-check.mjs --yes-write-to-live --sdk=<that dir>
    ```

    - **`node:module`'s `registerHooks` maps the gstatic SDK URLs onto a local install**, which is
      what makes `js/firebase-backend.js` ITSELF the thing under test rather than a lookalike. The
      firestore import lands on `tools/live-check-firestore.mjs`, which re-exports the real SDK with
      counters — explicit exports shadow `export *`, so the app gets the wrappers and nothing else.
    - ⚠️ **THE VERSION IS PART OF THE CLAIM.** The app loads **10.12.2**; the project's own
      `node_modules` holds 12.18.0 for the rules tests. Install the old one somewhere else and point
      `--sdk` at it — the tool prints what it used and warns when it is not 10.12.2.
    - 🚨 **NODE HAS NO `localStorage`, AND THAT IS THE TRAP.** The incremental sync's cache lives
      there, so without a stand-in `localShardCache()` returns a no-op, **every read becomes a full
      read, the mechanism being measured never runs — and every correctness assertion still passes.**
      A green run measuring nothing; §0.14 one level up. It also has no IndexedDB, so
      `persistentLocalCache()` is made to throw and the app takes its own documented fallback.
    - ⚠️ **COUNT THE DOCUMENTS, DO NOT ASSERT THE ROWS.** The rows are right on the expensive path
      too. `docsReturned` is the only thing that separates the two, and against a real server it is
      the billed number.
    - 🚨 **A POSITIVE RESULT NEEDS ITS NEGATIVE CONTROLS.** "The publish was accepted" proves the
      rules let something through, not that they are the NEW ones. The tool refuses an unnamed key, a
      public document carrying body weight, an over-cap list and a legacy audience id — each by a
      clause that did not exist before the deploy under test.
    - 🛑 **THROWAWAY ACCOUNTS, AND CLEAN UP ONLY WHAT YOU MADE.** It prints both uids on every exit;
      `firebase firestore:delete --recursive users/<uid> --force` removes the residue (an emptied
      `collections/*` document cannot be deleted by its own client — the rules say `delete: if false`
      — so a crash leaves husks only an owner credential can clear). **Never clean up to zero**: the
      project holds real accounts, and `docs/firebase-setup.md` records the day a stale "zero users"
      sentence went into a brief for an agent deleting test accounts.

17. **🔒 A RULE GUARDED BY ITS WEAKEST REASON GETS OVERTURNED BY WHOEVER SOLVES THAT REASON** —
    2026-09-06, moved here from a dated summary on 2026-09-17 because it is a rule about method and
    was sitting where §0.3 says a rule must not sit.

    §9 and `exercises.js` both gave *"the app does not record the height"* as why two exercises are
    unrankable — **a problem the app can obviously solve** — when the binding reason was an unfixable
    mismatch of measurement bases. A later session read it and set out to build the field. **State the
    binding reason first**, and put the solvable one second or not at all. Corrected in three places,
    including the test that exists to stop somebody filling those fields in.

18. **⚠️ A GREEN BROWSER AUDIT WITH A ZERO TEXT-NODE COUNT IS NOT A PASS** — 2026-09-06, moved here
    on 2026-09-17 for the same reason as §0.17.

    The first run measured **404 pages end to end** — a stale server on the port — and reported zero
    contrast failures and zero overflow across all 128 routes. **Check the node count before reading
...
21. **🔒 FOUR WAYS OF WRITING AN ASSERTION, ALL LEARNED THE HARD WAY** — moved here from
    `progress.md`'s Render-tests row on 2026-09-22, because it is method rather than coverage and it
    was sitting in a reference row nobody re-reads. Each of these is a shape, not an anecdote:

    - **Assert an ABSENCE when absence is what makes it safe to ship.** The first exercise-picture
      assertion checks that with no art bought, *nothing renders* — which is the only thing that made
      it safe to ship the feature ahead of the art.
    - **Give the fixture the case that can fail.** The "remove a person" control is asserted with
      **two guests on the bar, because with one it passes however the code is written.** A fixture
      that cannot distinguish the right answer from the wrong one is not a test. (The same fault, on
      a different axis, is the female-map lesson: every fixture was male, so a bug about women could
      not show — §9's own note, 2026-09-09.)
    - **Assert by CLICKING, not by reading the tree.** Back buttons and swap rows are driven, which
      is the only version that caught **five inert back buttons** — every one of them present,
      correctly labelled, and wired to nothing.
    - **Hold the NODE across the change when the point is that it survived.** A nudge must repaint a
      set row *in place*; the assertion keeps a reference to the row and checks it is the same node
      afterwards, because a rebuild would destroy the input under the user's finger and no
      content-based check can see the difference.

    ⚠️ **And one budget rather than a presence check**: the muscle panel is capped at 40 words. It is
    the only assertion in this project that can catch prose *piling back up* — everything else checks
    that a thing is THERE, and none of those can ever fail because a screen got wordier.

22. **🚨 FIRESTORE REFUSES A NESTED ARRAY, AND BOTH BACKENDS THE TESTS USE ACCEPT ONE — 2026-09-27.**
    Firestore cannot store an array whose elements are arrays. `setDoc()` rejects the **whole
    document**, so one bad field loses an entire collection rather than one row.

    **What it cost:** rep prescriptions were stored as one `[lo, hi]` pair per set, so `reps` was
    `[[3,5],[8,8]]`. Copying a programme wrote the system row (scalars — fine), then threw on the
    first `saveWorkout()`; the `workouts` document was never created, every later read returned `[]`,
    and the screen said *"no workouts yet"*. Tim reported it as a broken copy. **It was never about
    presets** — any workout with per-set prescriptions was unsaveable to the cloud, including a
    hand-built one, from the day the feature shipped. The stored shape is `{lo, hi}` now
    (`js/set-reps.js`); readers still take either.

    🛑 **THE REASON IT SHIPPED IS THE TRAP, AND IT GENERALISES BEYOND THIS FIELD.** `LocalBackend` is
    `JSON.stringify`, which nests arrays happily, and the Firestore **test double's `setDoc` stores
    whatever object it is handed without type-checking**. So the only thing in existence that
    enforces this constraint is the real server, which no suite reaches — every test passed, on both
    backends, for nine days. ✅ **`tests/data-layer.test.mjs` now walks every collection after
    copying all nine presets and fails on any array inside an array**, with a vacuity guard proving
    the walker finds one when it is there. **Keep it general — the next nested array will not be in
    `reps`.**

    ⚠️ **The general form, and it is §0.16's and §0.18's again in a third costume: a test double that
    is more permissive than the real thing turns a guarantee into an assumption.** When a double
    exists, ask what the real one refuses that the double allows, and assert *that* against the data
    rather than against the double.

    🚩 **AND A LOCAL REPRODUCTION CANNOT CLEAR A CLOUD PATH.** This bug was called "not reproducible,
    the copy is fine" off a Node run against `LocalBackend` — a claim about the wrong machine, made
    to Tim, twice. Anything that behaves differently on the two backends (write validation, latency,
    and every race that latency opens) is unproved until it runs where it fails.
...
| `js/research-topics.js` | Not a doc. **The Research tab's topics — read its header before adding one.** Every claim names a source, every source is defined ONCE, every topic states its own limit, and nothing goes on that screen below "limited evidence". ⚠️ **Anything added here must be added to `docs/research.md` first**, with its grade and its limitations. 🚨 **REBUILT 2026-09-22 BY THE PARALLEL AGENT (`462e8a8`) AND THIS ROW HAS NOT BEEN RE-DERIVED FROM IT** — ~~eleven~~ **nineteen topics in five sections**, plus tag facets and per-topic hooks. `docs/research-plan.md` is their write-up; read the module's own header and that file rather than this row |
...
| `js/body-art.js` · `js/body-art-female.js` | Not docs. **GENERATED by `tools/build-body-art.py` — never hand-edited.** One traced path per muscle group per view plus the silhouette; the keylines, striations and shading are in `img/ink-*.webp`, not in these. 🚨 **Two figures since 2026-09-22, picked on the profile's sex, and the pair must stay in step** — same groups, same views, or a muscle ranks on one person's map and reads "nothing recorded" on another's. The tool refuses to emit a mismatch and `tests/data-layer.test.mjs` pins it view by view. ⚠️ **Read the tool's header before regenerating either**: the two SOURCES are read completely differently (colour field vs line art) and only the front-ends differ — the smoothing, the piece-count guard and the trace are shared, which is what keeps the outline-quality fix in one place. **Check the male output is byte-identical after touching anything shared.** |
...
| `js/set-reps.js` | Not a doc. **A planned set as a rep prescription** (2026-09-20, D33) — pure, and the sibling of `set-targets.js`. 🚨 **THE STORED SHAPE IS `{lo, hi}` PER SET AND IT MUST NOT BE "TIDIED" BACK INTO `[lo, hi]`** — that pair is an array inside an array, which **Firestore rejects outright, losing the whole `workouts` document** (§0.22, and the bug Tim reported on 2026-09-27 as a copied programme arriving empty). `normalizeReps()` is the one boundary that emits it; `normalizeRepSpec()` accepts the stored map, the legacy pair and a bare number, which is what keeps every row written before then readable. ⚠️ **`expandRepSpec()` decides "per-set list" by looking at the FIRST ENTRY**, so it must recognise both shapes — testing only for an array made a stored list read as a single range, and that misreads on the *second* load of a workout rather than the first. ⚠️ **`origin.reps` carries the same shape** and converts with it in `normalizeWorkout()`. 🛑 Read the header before touching the constants: `RIR_LOW`/`RIR_HIGH` change every prescribed weight in the app, and a prescription is a QUOTATION — never clamped to D5's fifteen |
| `js/public-figures.js` | Not a doc. **Famous lifters as recorded lifts** (2026-09-27) — GENERATED from three research passes, so correct a number only against its cited source. Rated by `buildStrengthShare()` exactly like the reader. 🚨 **Each muscle as of its freshest lift** (`figureStrength()`), or a record spanning years draws colourless. Raw, sourced, bodyweight on the day; nothing inferred from appearance |
...
| `js/schedule.js` | Not a doc. **A system's optional weekly or cycle plan** (2026-09-16) — pure: no DOM, no store, **no clock**. `store.js` validates with it and `views-workouts.js` draws with it, so neither can drift from the other's idea of what a slot may hold. 🛑 **Read its header before adding anything to it**: the plan is DISPLAY ONLY on Tim's own answer, so nothing in it computes "what day is it today", and a function that did would be the whole of the feature he declined sitting one call site away from being switched on by accident — `trainingDays()` was written and deleted for exactly that reason. ⚠️ **Rest and "nothing planned" are different slot states** and must stay so: deleting a workout empties its day rather than resting it, because inventing a rest day is Rule 6 |
| `js/profile-shape.js` | Not a doc. **The sections a profile is made of, drawn once for two people** (2026-09-16) — the head, the stat tiles, the body block, the best-lifts list and the calendar, used by `#/me` and by a friend's page. ⚠️ **Read its header before adding a section**: the *shape* is shared and the *row data* is built per subject on purpose, because "where does this number come from" is a genuinely different question about a friend than about you, and flattening that behind a `friend` flag is how a caveat gets printed over the wrong person. It carries two `NO_NUMBER` sentence maps for exactly that reason — "nothing recorded for this muscle yet" is an instruction on your own page and a statement about somebody else on theirs |
| `js/ratio-sigma.js` | Not a doc. **How much each conversion ratio DRIFTS across the published strength range — GENERATED by `tools/build-ratio-sigma.mjs`, never hand-edited.** σ_drift = \|ln r₈₀ − ln r₂₀\| / 1.68. 🚨 **It is the measured half of the uncertainty only**; gearing and sourcing are judged and live in `sigmaFor()`, because every row of a machine's published page is the same population on the same machines and the drift cannot see another gym's lever arm |
| `js/rep-sigma.js` | Not a doc. **How uncertain the rep → 1RM conversion is, per rep count — GENERATED by `tools/build-rep-sigma.mjs`, never hand-edited** (2026-09-20). σ is the DISAGREEMENT between the seven classical formulas in `docs/research.md` §1.2, each normalised by its own f(1): 0 % at 1 rep, 2.6 % at 6, 3.1 % at 8, 4.2 % at 10, 5.9 % at 12, 9.6 % at 15. 🆕 **AND IT RUNS TO 25 SINCE 2026-09-23** — 18.8 % at 20, 34.1 % at 25 — because the muscle map reads that far now (`MAX_MAP_REPS`). ⚠️ **Rows 1–15 are unchanged to the last digit**, which is what made "nothing already rated moves" a measurement. 🛑 **It stops at 25 and that is a statement**: at 30 reps σ is 61 %, past `SIGMA_MAX`, where the clamp eats it and every long set prices identically. 🚨 **It replaced a hand-typed ladder that had no source for any of its numbers** (1.00 / 0.95 / 0.85 / 0.70 / 0.45 / 0.25, where one extra rep could cost a set 36 % of its weight at a step boundary) — read the tool's header before touching it, because the normalisation by f(1) is the load-bearing part: a one-rep set converts nothing, so its doubt must be zero, and the formulas' raw disagreement at r = 1 is an artefact of how they were fitted. ⚠️ **It is ONE term, combined in quadrature inside `readingSigma()`** in `muscle-evidence.js` — not the total error of an estimate, and it does not know whether a set was taken to failure (D28) |
| `tools/strength-level-data.mjs` | Not a doc. **Every Strength Level row this project has pulled**, male at 180 lb and female at 140 lb, with the equipment notes that decide whether a number is per dumbbell or a total. 🚨 **It lived in a temp scratchpad until 2026-09-15 and was nearly lost with the session that pulled it — the ratio table cannot be re-derived without it.** `tools/strength-level-map.mjs` says which library exercise each page describes, and ⚠️ **a row in there is a claim that the page and the exercise are the same movement under the same load convention**; three are deliberately absent for exactly that reason |
...
| `tools/live-check.mjs` | Not a doc, dev-only, and **the only thing in this repository that writes to the LIVE project** — refuses to run without `--yes-write-to-live`. 🚨 **Read §0.16 before running it.** It redirects the app's gstatic imports onto a local SDK so `js/firebase-backend.js` itself is exercised, counts the documents Firestore actually returns, and answers the two questions no test can: are the DEPLOYED rules the ones this client expects, and what does a sync cost. ⚠️ **It was reconstructed from one sentence in `docs/firebase-setup.md`** because the 2026-08-15 version of it was a scratchpad script that got deleted — hence it lives here now. `tools/live-check-firestore.mjs` is its counting wrapper |
...
| `js/session-draft.js` | Not a doc. **The workout in progress, on disk** — `liveDraft()`, the one copy of "a draft only lives for the day it was started", and since 2026-09-07 **`setIsRecorded` / `hasNumbers` / `draftRecordedSets()`, the one copy of "was this set really performed"** (four callers: the save path's filter, the save screen, the conflict screen and the bar's bin). ⚠️ **Read its header before moving anything back into the runner**: the bar on every screen and the runner have to answer "is one open?" identically, and the bar cannot import the runner |
...
├── Female_Muscle_Groups_*.png  the FEMALE body-map source, one per view, Tim's
│                               own (2026-09-22). GIT-IGNORED (/*.png). ⚠️ THE
│                               ART CANNOT BE REBUILT WITHOUT THESE THREE FILES
├── img/ink-front.webp          the body map's ink layer, ~100 KB each. GENERATED
│   ├── ink-back.webp           the only binary assets in the app
│   ├── ink-front-female.webp   ⚠️ written at the SOURCE's own scale, not the
│   └── ink-back-female.webp    figure box's — the SVG scales the mask either
│                               way, and the back drawing is 1067px tall against
│                               a 1527 box, so upsampling would cost 40 % of a
│                               precached file to carry no more detail
├── tools/build-body-art.py     regenerates js/body-art*.js + img/ink-*.webp from
│                               the sources. Dev-only; needs pillow/numpy/scipy/
│                               potracer. `--only male|female` for one of them
│                               ⚠️ TWO FRONT-ENDS, ONE BACK-END, SINCE 2026-09-22.
│                               The male sheet is a COLOUR FIELD (a muscle is a
│                               hue); the female is MONOCHROME LINE ART (a muscle
│                               is a region the white keylines enclose), so
│                               segment() and segment_female() are two readings
│                               of a picture. Everything after them —
│                               smooth_fills, the piece guard, trace, emit — is
│                               SHARED, because that is where the outline quality
│                               lives and a second copy is a second place for the
│                               crenellation bug to return. The male output
│                               regenerates BYTE-IDENTICAL; check that after
│                               touching anything shared.
│                               ⚠️ CUTS_F and IGNORE_F are hand-placed and the
│                               female drawing needs both: it has no line between
│                               shin and foot on the front (a 15px anatomical
│                               waist, not a faint line), and its head/hands/feet
│                               are the same grey as a muscle rather than blank
...
│   │                           ⚠️ AND absorbThisDevice() SINCE 2026-09-08 —
│   │                           creating an account merges this browser's
│   │                           local rows into it, with no button. The
│   │                           condition is CREATED, never SIGNED IN:
│   │                           signing in reaches an account with its own
│   │                           history, possibly on somebody else's phone.
...
│   │                           ⚠️ AND SINCE 2026-09-15 THE BLEND IS
│   │                           INVERSE-VARIANCE — sigmaFor() swaps `quality`
│   │                           for 1/σ², and deliberately does NOT touch
│   │                           depth, agreement or the candidate sort,
│   │                           whose fitted constants belong to the old
│   │                           scale. Every confidence is unchanged
│   ├── ratio-sigma.js          HOW FAR EACH RATIO DRIFTS across the published
│   │                           strength range — GENERATED, the measured half
│   │                           of σ. tools/build-ratio-sigma.mjs
│   ├── rep-sigma.js            HOW FAR THE REP→1RM CURVE IS EXTRAPOLATING —
│   │                           GENERATED, do not hand-edit. The disagreement
│   │                           between research §1.2's seven formulas, each
│   │                           normalised by its own f(1) so a single is 0 %.
│   │                           2.6 % at 6 reps, 4.2 % at 10, 9.6 % at 15.
│   │                           🚨 REPLACED A HAND-TYPED LADDER WITH NO SOURCE
│   │                           (1.00/0.95/0.85/0.70/0.45/0.25). Combined in
│   │                           quadrature by readingSigma(), which is separate
│   │                           from sigmaFor() on purpose — that one is the
│   │                           CONVERSION doubt and a key lift has none of it.
│   │                           tools/build-rep-sigma.mjs
...
│   ├── research-topics.js      THE BASICS, TAUGHT — pure. NINETEEN topics in
│   │                           five sections, sources each defined ONCE, a
│   │                           confidence level per topic and a stated limit
│   │                           on every one.
│   │                           🚨 REBUILT 2026-09-22 BY THE PARALLEL AGENT
│   │                           (462e8a8) — facets and hooks, and eight more
│   │                           topics on top of the original eleven. Neither
│   │                           this tree nor docs/state.md has been re-derived
│   │                           from it; docs/research-plan.md is their write-up
...
│   ├── body-art-female.js      the same, for the female figure (2026-09-22).
│   │                           582x1527 against the male's 506x1527 — same
│   │                           HEIGHT on purpose, so neither body is drawn
│   │                           smaller than the other
│   ├── body-map.js             composes the fill paths + the ink masks
│   │                           ⚠️ PICKS A FIGURE ON opts.sex SINCE 2026-09-22,
│   │                           and the sex is WHOSE BODY IS DRAWN rather than
│   │                           who is reading — a friend's map, their Volume
│   │                           figure and each compare column are theirs.
│   │                           bodyAspect(sex) replaced BODY_ASPECT: the two
│   │                           drawings are different shapes, and a container
│   │                           built to the wrong one shrinks the figure and
│   │                           pads the sides rather than cropping it
...
│   ├── profile-records.js     "WHAT ARE MY BEST LIFTS, EVER?" (2026-09-10) —
│   │                           pure. The measured set leads and the estimate
│   │                           follows, labelled (Rule 5); ordered by DAYS
│   │                           TRAINED, because there is no honest ranking of
│   │                           a 405 deadlift against a 40 lateral raise
│   ├── profile-ranking.js     "WHERE DO THEY SIT, AGAINST PEOPLE LIKE ME?"
│   │                           (2026-09-12) — pure. The core eight plus the
│   │                           rest, each an estimated 1RM with a level and a
│   │                           band. Says WHY a row has no number rather than
│   │                           leaving a hole
│   ├── rep-decrement.js       HOW REPS FALL FROM SET TO SET at one load
│   │                           (2026-09-13) — pure, and the only fatigue the
│   │                           captions may honestly carry. ⚠️ EVERY
│   │                           MULTIPLIER IS ≤ 1, so a wrong constant can
│   │                           only make the caption easier to beat
│   ├── model-version.js       WHICH STRENGTH MODEL A STORED NUMBER CAME FROM
│   │                           (2026-09-13) — twelve lines. A goal freezes a
│   │                           target weight; when the ratios or medians move,
│   │                           this is what says the frozen number predates
│   │                           the change rather than disagreeing with it
│   ├── feedback.js            A NOTE TO THE DEVELOPER (2026-09-04) — pure.
│   │                           🛑 TEMPORARY by design; the developer is a
│   │                           hard-coded uid in firestore.rules, not a
│   │                           hidden screen
│   ├── set-targets.js        🆕 A PLANNED SET AS A % OF A MAX, 2026-09-18 —
│   │                           pure. The percentage is of THEIR OWN BEST
│   │                           RECORDED SET on that lift, never the muscle
│   │                           map's cross-muscle estimate: that path is
│   │                           gated four ways because its number is walked
│   │                           up to a bar, and a percentage of it stacks
│   │                           two inferences. Body-weight and assisted
│   │                           lifts refused (body-inclusive max, added-only
│   │                           field — D30). Rounds DOWN. 100 % is a ceiling
│   ├── plates.js             🆕 WHICH DISCS MAKE THIS NUMBER, 2026-09-18 —
│   │                           pure. The line under the weight stepper.
│   │                           ⚠️ Greedy is minimal only over a CANONICAL
│   │                           set and "each divides the next" is FALSE
│   │                           here, so the lb inventory drops the 35 and
│   │                           the kg one must keep its 15 — measured by
│   │                           exhaustive DP, re-run by the suite. A weight
│   │                           no plates make prints NOTHING
│   ├── schedule.js            🆕 A SYSTEM'S OPTIONAL WEEKLY OR CYCLE PLAN,
│   │                           2026-09-16 — pure, no clock. Two kinds ('week'
│   │                           IS Mon–Sun; 'cycle' is N slots that repeat),
│   │                           three slot states (a workout, Rest, nothing).
│   │                           🛑 DISPLAY ONLY: nothing computes what day it
│   │                           is today, and next-workout.js does not import
│   │                           it. Rest and "nothing planned" are different
│   │                           words on purpose
│   ├── profile-shape.js       🆕 THE SECTIONS A PROFILE IS MADE OF, 2026-09-16
│   │                           — head, stat tiles, body block, best lifts,
│   │                           calendar. ONE module, TWO subjects: #/me and a
│   │                           friend's page. The shape is shared; the row
│   │                           data is built per subject, because "where did
│   │                           this number come from" is a different question
│   │                           about somebody else and a `friend` flag would
│   │                           print one person's caveat over the other's
...
- **⚠️ `shardDiff()` COMPARES `JSON.stringify(row)`, AND FIRESTORE HANDS MAP KEYS BACK IN AN ORDER OF
  ITS OWN.** Measured 2026-09-17 against the live project: a row written
  `id,date,workoutId,workoutName,…` came back `id,date,isBenchmark,workoutName,…`. Same data,
  different string, so every row reads as changed. 🛑 **This is not a bug and must not be "fixed":**
  `store.js` does a read-modify-write, so the rows handed to `write()` ARE the objects the read
  returned and the memo was built from, and only genuinely changed rows are written — proved by
  counting the documents each write issues (2 of 4, against 4 of 4 for rows rebuilt from literals).
  ⚠️ **The consequence, and it is about cost rather than correctness**: any path that RECONSTRUCTS
  rows — restoring from a downloaded backup file is exactly that shape — pays one write per row
  instead of none, once. Nobody has asked for anything here.
...
Workout     id, name, systemId, isBenchmark, order?, presetKey?,
            ...
            ── `targets`: one percentage per planned set (2026-09-18), 30–100 in
               ...
               ⚠️ RECONCILED AGAINST `sets` ON EVERY READ, in normalizeWorkout():
               padding repeats the last value (a ramp's last rung is the working
               weight) and truncation drops from the end. One unusable row drops
               the WHOLE prescription rather than defaulting that set — half a
               ramp is not a ramp. js/set-targets.js
...
- ⚠️ **`emptyState()` AND `confirmSheet({ message })` TAKE A PLAIN STRING**, so there is nowhere to
  hang a dot in either without changing `ui.js`. Every empty state and every confirmation is
  therefore reshape-only under this rule. Worth knowing before planning a screen's pass.
- 🚨 **AND SOMETIMES THE ANSWER IS DELETE — 2026-09-08, the Account screen.** Two of the wordiest
  blocks on it were **saying something the screen already said**: a "Signed in" card explaining sync,
  directly under the avatar card that prints the email; and, on the demo screen, three paragraphs
  restating the demo bar `app.js` prepends to *every* screen. **Putting a duplicate behind a ? keeps
  it, and it was never worth keeping.** Ask what else on this screen already says it before reaching
  for the dot. Same day, same screen: *"Edit to move or resize the circle"* sat eight pixels under a
  button labelled **Edit**.
- 🚨 **WHERE THE DOT SITS IS PART OF THE RULE, AND IT IS TIM'S** — 2026-09-08, after the first
  version right-aligned them: *"If something has a label like 'Just looking around' above demo
  account button, the question mark should be right next to the just looking around text, not on the
  right side. this way it's easy to know where to get information."* So **a `.section-label` or a
  `<label>` in a `.help-line` does NOT stretch** — the dot sits against the words. A `.field-help`
  or a `.note` still does, because there the dot follows a sentence that already fills the row.
  ⚠️ **`flex: 0 1 auto`, never `flex: none`**: a long label ("Your data — only in this browser") on a
  360px screen must still be allowed to shrink, or it is pushed off the side.
...
| D5 | **e1RM must be rep-range honest.** Full confidence 2–10 reps, flag 11–15, don't normalise above 15. | Formulas degrade badly above ~10 reps. Built — and **enforced in ranking as `MAX_EVIDENCE_REPS = 15`** since 2026-08-16. It was not, and a 135×25 burnout set extrapolated to 258 lb, beat a real 205×5 top set and promoted a muscle a whole level. Benchmarks get no exemption. 🔄 **NARROWED 2026-09-23 — THERE ARE TWO CEILINGS NOW AND D5 IS STILL 15.** The MUSCLE MAP reads to `MAX_MAP_REPS` (25); `MAX_EVIDENCE_REPS` is unchanged and `isRankableSet()` is byte-identical, so the charts, the personal bests, progression, the comparison screen and `setE1rm()` all still stop at 15. 🔒 **The line between them is not caution, it is arithmetic: the map blends at 1/σ² and can therefore PRICE a long set out of its own uncertainty, where a screen printing one number has nothing to pay with.** A 25-rep set carries a tenth of a 15-rep set's blend weight, measured by `js/rep-sigma.js` rather than chosen. 🚨 **D5's own regression case now passes for a different reason** — the 135×25 is ADMITTED and loses at the seat, so the guard moved from the gate to the seat rule and its assertion had to be rewritten to pin the set is present before pinning that it changes nothing. |
...
🚨 **D32 IS RECORDED ON 2026-09-16, AND IT IS A WIDENING RATHER THAN A NARROWING — the first one
this project has made:** *a published document carries the owner's IDENTITY (name, photo, gender,
age) and a list NAMING OTHER PEOPLE (`connections`), in both audiences; body weight remains the one
field the two documents disagree about.*

- **Why it exists.** Tim asked for a friend's page to be their profile — *"the 'your body' details,
  but leave out the weight (only show gendar and age)"* — and for their friends list to be openable
  so a reader can walk on. None of that could be rendered, because the document did not carry any of
  it. This is not a screen decision; it is a change to what every account publishes.
- 🚨 **`connections` IS THE PART THAT IS GENUINELY NEW, and it does not follow from D29.** D29 says
  public means anybody signed in reads what a friend reads — a statement about the owner's own
  training. This is the first field that names **other people**, so on a public account the whole
  signed-in world can read who you are connected to. It was taken deliberately, with the argument on
  both sides written at the top of `js/social.js`, and **the reversal is one line** (`!forPublic`,
  the same gate body weight already uses). ⚠️ **It grants nothing**: `viewers` is untouched, and no
  row carries a face — each friend's photo is read from that person's own document, because fifty
  avatars is ~4.5 MB against a 1 MB ceiling and a document that outgrows it stops publishing.
- ⚠️ **THE AGE IS PUBLISHED, NEVER THE BIRTH YEAR.** `getProfile()` derives an age on every read
  because a stored one goes stale in silence, and a birth year is the more identifying of the two to
  hand a stranger. The accepted cost is that the figure drifts by up to a year between republishes;
  the document carries `publishedAt`, so a reader can see how old it is.
- 🔒 **THE GUARD WAS EXTENDED RATHER THAN WORKED AROUND.** `assertAudienceClean()` now looks
  **inside** `profile` and inside every `connections` row, so `profile.bodyWeight` and
  `profile.email` are refused at the publish site and a connection row carrying anything but
  `{uid, name}` throws. That matters because `getProfile()` returns body weight in the *same object*
  as gender and age — the leak here was one spread operator away at the call site.
- 🚨 **`firestore.rules` HAD TO SHIP WITH IT, NOT AFTER IT.** `validProjection()` pins the document
  with `hasOnly`, so a client publishing a key the rules do not name has **every publish denied** —
  silently, because `republish()` is fire-and-forget and its throw is swallowed at every call site.
  The symptom would have been "nobody's page updates any more" with nothing on any screen saying so.
  Mutation-checked: removing `connections` from that list turns the publish assertions red.
- ⚠️ **ABSENT IS NOT EMPTY, and a reader must keep telling them apart.** Every account published
  before this has no `connections` key at all; `[]` means "nobody". `healStalePublish()` republishes
  such a document the next time its owner opens the app, and until then their page says their app
  has not published it yet rather than claiming they have no friends.
...
|---|---|
| the conversion ratios are estimates and some are shaky | 🔄 **Largely closed 2026-09-15.** ~25 were corrected on -14 and the rest re-derived on -15, after fixing a check that could not read a `{m, f}` pair: **85 of 105 entries are now inside 5 % of the published figure**, against 38. Three are refused because their published page is a different implement, and the refusal is written into the table. ⚠️ **Machines are still the weak case and the reason is now stated rather than felt**: `js/ratio-sigma.js` measures how far each ratio drifts across the strength range, and a machine's can be perfectly flat while its GEARING still varies by brand — which no published table can see, because every row of it is the same population on the same machines. That doubt is its own term in `sigmaFor()` |
...
| high-rep extrapolation is not fixed | 🔄 **THE LADDER HALF IS FIXED AND MEASURED AS OF 2026-09-20 — see the end of this row; the rest of the problem is still open, because nothing in the app knows whether a set was taken to failure (D28) and that is the largest single source of error in the whole model.** Before that it read *partly* — the rating prefers sets at ≤ 8 reps where they exist, so the longest set no longer leads by default. 🔄 **AND SINCE 2026-09-20 THAT PREFERENCE CAN NO LONGER PUNISH YOU FOR DOING MORE.** It was a filter, not a weighting, and `seatCredit` contains no weight term at all — so a set heavier AND longer than the seated one could not win the seat, only lose it. Tim caught it on his own Back: a Lat Pulldown 50×6 held the seat while an 85×12 three weeks newer contributed **0.00 lb**, and the discarded set implied 131 against the seated 66.7. **A set that is ≥ in BOTH weight and reps now supersedes**, re-read at the weaker set's rep count — heavier at the same credibility, and the conservative half of what it proves. Heavier-but-shorter is untouched: that is a real trade and the credibility comparison still decides it. `dominate()` in muscle-evidence.js. 🚨 **IT IS CALLED TWICE AND BOTH ARE REQUIRED** — once on each exercise-DAY before `betterSameDay` collapses it, and once at the seat. Shipping it at the seat alone left the reporter's own case broken and looked green in every suite: on the day he pulled 85×12 he had opened with 65×8, and the per-day collapse uses the *same* weight-blind comparison, so the heavy set died one step early. **A rule enforced at one of two places that make the same comparison is not enforced.** ⚠️ **The demo year never produces that shape**, so the golden table did not move when the second half went in — there was no cover until a fixture was written for it, and the check not done was driving the change against the reporter's own described case (*"my third set"*) rather than a simplification of it. 🛑 **IT MUST RUN NO EARLIER THAN THE DAY COLLAPSE** — three attempts in `buildObservations()` collapsed repeat sets, folded a progressive year onto a handful of dates (720 observations → 44), and **disabled the typo quarantine** (a mistyped 2050×5 dominates everything, drags every real set up to its weight, and leaves the day-screen nothing to disagree with — Chest read Elite at 2282 lb). A rule that rewrites evidence runs after everything that screens it. 🚨 **AND THE SAME DAY THE DISCOUNT ITSELF STOPPED BEING A GUESS.** Tim asked the question nobody had — *"I don't know where the 45% trust is coming from… I want higher-rep counts to count towards the 1RM estimation almost the same as lower-rep counts, unless there is a good amount of data that agrees with each other recorded for that 1RM already, so using the low rep count just adds precision and confidence"* — and the answer was **nowhere**. `repFactor()`'s 1.00 / 0.95 / 0.85 / 0.70 / 0.45 / 0.25 was hand-typed, no tool fitted it, `docs/research.md` does not contain it, and its own comment cited the research only for the SHAPE. It was also a staircase: eleven reps worth 36 % less than ten, thirteen 44 % less than twelve, flats between, on literature that says *degrades above ten* rather than *worth half*. **σ_rep is what replaced it** — `js/rep-sigma.js`, GENERATED by `tools/build-rep-sigma.mjs`, measuring how far the seven classical formulas of research §1.2 DISAGREE about the r → 1 extrapolation, each normalised by its own f(1) first so a single (which converts nothing) comes out at exactly 0. **0 % at 1 rep, 2.6 % at 6, 3.1 % at 8, 4.2 % at 10, 5.9 % at 12, 9.6 % at 15**: smooth, monotone, and the same technique `tools/build-ratio-sigma.mjs` already applied to the conversion ratios, so the second input that was carrying a judgement now comes from seven independent authors instead of this project. ⚠️ **IT LIVES IN `readingSigma()`, WHICH IS A SECOND FUNCTION RATHER THAN A BIGGER `sigmaFor()`, AND A FAILING TEST IS WHAT INSISTED**: `sigmaFor()` is documented and asserted as the CONVERSION doubt — *"a key lift carries none of it, its ratio is 1.00 by construction"* — and folding the rep term in made that sentence false while every caller still read it as true. Two doubts, two functions, combined **in quadrature**, and the caller says which it wants. 🚨 **THE QUADRATURE IS WHAT DELIVERS HIS ASK WITH NO SPECIAL CASE, AND IT IS MEASURED**: on a machine, where the conversion is already doubtful, a 12-rep set carries **86 %** of an 8-rep set's weight — *"almost the same"*, because the rep count is not what the doubt is about; on the key lift, where the ratio is exact and σ_rep is nearly all that is left, **57 %**, so low reps genuinely *"just add precision"*. The ladder said **53 % in both cases**. One extra rep now costs at most **21 %** of a set's weight against the ladder's **44 %**. ⚠️ **The key lift's 57 % against the ladder's 53 % is close, and that is the honest result rather than a failure** — where the rep count IS most of the doubt a measured σ and a hand-drawn ladder happen to agree; the ladder's fault was never that every number was wrong, it was that it gave the SAME answer everywhere. 🛑 **`repFactor` SURVIVES ONLY AS THE SEAT COMPARISON.** It is out of the blend weight (`evidenceWeight / quality / repFactor(reps)` divides it back out) because charging a long set once as a multiplier and again as a variance is the same doubt twice — but it still decides which of an exercise's own sets speaks, which is a different question from what that set's number is worth. Recency, fatigue and the benchmark bonus stay in the blend: none of them is a statement about how far the curve is extrapolating. Golden re-baselined, small and in **both** directions because this re-weights rather than re-reads (Core +6.5 %, Quads −0.6 %, Shoulders −0.7 %, Back +0.2 %, the rest under 0.3 %, Glutes/Forearms/Traps unchanged); every observation and contributor count unchanged, and Quads' confidence rose 0.8510 → 0.8793 because the seats it now blends agree more closely than the ones the filter forced on it |
...
- ~~**Core, Neck and Cardio can never be ranked**~~ ⚠️ **STALE SINCE 2026-09-04 AND CORRECTED
  ...
  🔄 **AND NECK LEFT IT ON 2026-09-23. `UNRANKABLE` IS `['Cardio', 'Activity']` — TWO LIBRARY
  SHELVES — AND NO MUSCLE DRAWN ON THE FIGURE IS IN THAT STATE ANY MORE.** 🚨 **Not a loosening,
  and the pattern is the thing to carry: a muscle leaves this list when somebody FINDS A PAGE,
  never because the map looked empty.** Core left because a Cable Crunch table was pulled; Neck left
  because Strength Level publishes neck curl and neck extension, unlinked from their own browse
  index, while **four places in this repo had recorded their absence as a permanent fact**.
  ⚠️ **`docs/research.md` §14.6 had warned about exactly this** — it filed the plank and sit-up
  norms as *unchecked rather than rejected*, because *"no honest source exists"* had already been
  wrong once. **Search past the first four queries before writing a permanent "nobody publishes
  this" into code.** 🛑 **The women's half of that page is still refused** — 55 people, and its
  body-weight slope runs backwards — so the female anchors are derived from a measured sex ratio
  and every Neck rating says so on screen. `docs/research.md` §17.
  🚩 **The hatch did not die with it**: what reaches it now is a muscle whose EVIDENCE was refused
  — every set longer than the map reads a maximum from, or work whose body-weight fraction nobody
  has measured — and those two wear different words, because one is cleared by a single heavier set
