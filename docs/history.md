...
### K. σ PER RATIO ENTRY, AND A BLEND THAT WEIGHS BY PRECISION (plan §6.4)

**Tim, given a second ranked answer:** *"just do 1 for now."*

**`q` says "believe this much" and is a judgement. σ is the same claim with a derivation.** The
measured half is now `js/ratio-sigma.js`, generated by `tools/build-ratio-sigma.mjs`:

> σ_drift = |ln r₈₀ − ln r₂₀| / 1.68

— how far an entry's ratio moves between the novice and advanced rows of the published table. A ratio
that holds across the strength range transfers to a stranger; one that doubles does not, and applying
one number to everybody is exactly that uncertain. **106 entries, 0.006 to 0.392.**

🚨 **IT AGREES WITH `q` ONLY LOOSELY — r = 0.63 — AND THE DISAGREEMENTS ARE THE POINT.** A machine
curl is judged at q 0.35 for gearing and drifts **0.016**, the flattest ratio in the file; a barbell
reverse curl is judged at 0.40 and drifts **0.354**, the widest. One of those is a suspicion about
hardware and the other is a measurement — so **gearing survives as its own term** rather than being
deleted because a drift came out flat. Every row of a machine's published page is the same population
on the same machines; the drift cannot see that another gym's lever arm is different.

🚨 **AND THE FIRST VERSION MADE THE KEY LIFT THE LEAST TRUSTED EVIDENCE A MUSCLE HAD.** A key lift's
ratio is 1.00 by construction, so it is not in the comparison table at all and fell to the default σ:
**a barbell bench went from 58 % of its own Chest rating to 4 %.** Measured before any of it shipped,
on a script that printed each muscle's weight shares — which is the only reason it was caught, because
every rating would still have looked plausible. It takes the sourcing floor and nothing else now, and
every muscle in the demo is led by its own key lift where one was trained.

**The blend swaps exactly one factor** — `quality` out, `1/σ²` in — leaving rep factor, recency,
fatigue and the benchmark bonus untouched, which is why the code reads `evidenceWeight / quality`
rather than building a second weight that could drift from the first.

🛑 **AND IT IS DELIBERATELY NOT USED FOR `depth`, `agreement` OR THE CANDIDATE SORT.** `depth` is
`1 − exp(−Σw / 1.5)` and that 1.5 is fitted to the scale `evidenceWeight` has today; 1/σ² reaches 400
at the floor, so dropping it in there would saturate depth to 1.0 for everybody and quietly delete a
term. ✅ **Every confidence in the golden table is unchanged to four decimal places**, which is the
check that the change stayed where it was put.

**The whole table moves −2.3 % to +2.8 %**, re-baselined with the cause written above it: Back
171.2 → 167.3, Shoulders 144.1 → 147.1, Traps 265.6 → 272.9, Triceps 163.5 → 161.4, and the rest
inside a percent.

⚠️ **A TEST CAUGHT THE MOST INTERESTING CONSEQUENCE, AND IT IS NOT A REGRESSION.** The 2026-08-24
finding — Tim's back session, where doing the pulldown fresh was worth **~60 lb** — now measures
**4.4 lb**. That is not the fatigue term weakening. σ for his three readings is 0.069 (dumbbell row),
0.118 (pulldown) and 0.307 (assisted pull-up), so the row carries 212 of ~295 total weight where `q`
gave it 0.60 of 1.44. 🚨 **Which is his original complaint fixed from the other end**: he reported
that adding the fatigued third exercise dragged Back from 212 to 145; it now reads **199.8**, because
the assisted pull-up — the one conversion in this file resting on an unstandardised machine linkage —
contributes 4 % instead of 20 %. **Order matters less because the answer depends less on the reading
order was distorting.** The assertion keeps the direction and the magnitude is recorded as a
measurement rather than legislated.

🛑 **NOT VALIDATED AGAINST A HUMAN, AND THE SIMULATOR CANNOT DO IT.** `tools/strength-sim.mjs` draws
each exercise's personal ratio departure as `RATIO_ERROR_AT_Q0 × (1 − q)` — a function of the very
number σ replaces — so scoring a σ-weighted rating against it would be marking the new scheme with
the old scheme's answer key. **Recorded on the constant.** What is asserted instead is what the
construction claims, including the load-bearing one: the same two disagreeing numbers land at 204.5
or 294.6 depending on which conversion is better established, where the old blend gave 225.0 both
ways. Mutation-checked in both directions.

✅ **AND THE SOURCE DATA IS IN THE REPOSITORY NOW.** `tools/strength-level-data.mjs` — every
published row this project has pulled — **lived in a temp scratchpad until today and was nearly lost
with the session that pulled it.** The ratio table cannot be re-derived without it.
