...
> one day's summary. 🆕 **There are 7 KB free now, on purpose** — three days collapsed rather than the
> usual one, and the Render-tests row cut from 8 KB of coverage to its method with the durable half
> moved to **§0.21**. That is a session or two of headroom, and it exists because every reset before
> this one spent its first twenty minutes fighting the budget instead of reading. **Spend it, then
> refill it** — do not let it go back to one byte.
...
every one), `data-layer` **2,861**, `render` **1,620**. ⚠️ **FOUR of those suites are a PARALLEL
