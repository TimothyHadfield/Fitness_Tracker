...
> **491 KB together as of 2026-09-20** (159 + 201 + 131), and none of it is optional. ⚠️ Each file has
> a byte budget with a test behind it — see §0.3.
> 🚩 **THIS FILE LIVES AT ITS CEILING AND HAS FOR FOUR SESSIONS RUNNING.** 2026-09-20 tripped the
> budget test **nine times** across four passes — every one of them fixed by collapsing something
> older, never by raising the number. **Assume you will have to collapse something before you can add
> anything**, and 🛑 **the fix is never to raise the
...
**Last updated:** 2026-09-20 — **a copied programme can now be told its original moved on.** One
open-ended instruction (*"start building or changing whatever you think should be built or
changed"*), so **Open work 36 was built** — the only fully-specified unstarted item, and it came out
of his own question the session before.
🆕 **`js/preset-updates.js`** (pure) + three stamps + a notice. 🛑 **THE COPY IS STILL A COPY** — the
live link refused on 2026-09-19 is still refused for all three reasons; what is new is that the app
can SAY what changed and take the half that is safe. 🚨 **THREE STAMPS**: `system.presetVersion`,
`workout.presetKey` (**after a rename** — the workouts recorded nothing before today) and
`exercise.origin` (**exactly as it arrived**), the last being **the only thing that tells "the
original changed" from "you changed it"**. 🛑 **THREE REFUSALS**: never deletes, never overwrites an
edit, appends rather than inserts. ⚠️ **`origin` HAD TO BE NAMED IN `normalizeWorkout()`** — the
third field caught by that warning, and the one that would have failed most quietly. ✅ **A test pins
a content HASH per preset** beside its `version`, because **a version somebody forgets to bump is
worse than no version** — every copy then believes it is current. ⚠️ **Every copy made before today
is unstamped, Tim's included**: content comparison only, every row "Yours to do", nothing applied,
and `copiedVersionOf()` returns **null, not 0** — that distinction is the whole branch.
**Details: Open work 36 and `docs/state.md`.**
🔒 **A MUTATION CHECK CAUGHT ITS OWN WEAK ASSERTION** — "their five sets are still five", the safety
...
🚨 **AND A FOURTH PASS: THE REP LADDER HAD NO SOURCE.** Tim: *"I don't know where the 45% trust is
coming from."* Traced — **nothing fits it**, `research.md` does not contain it, and its comment cites
the research only for the SHAPE. Six invented numbers deciding which set speaks for a muscle, with
**cliffs**: ten reps to eleven cost a set **36 %** of its weight, nine to ten cost nothing.
⚠️ **He was WRONG that all rep counts are equal** — Reynolds (2006) and Mayhew (2008) both find the
equations degrade above ten. **The preference was right; the magnitudes were invented.**
🆕 **`js/rep-sigma.js`** (generated by `tools/build-rep-sigma.mjs`) measures σ as the **disagreement
between the seven published formulas** in `research.md` §1.2, each normalised by its own f(1) so a
one-rep set has σ = 0. **The same technique `build-ratio-sigma.mjs` already used for ratios** — reps
were the last input still carrying a guess. 0 % at 1 rep → 3.1 % at 8 → 5.9 % at 12 → 9.6 % at 15.
🚨 **ADDED IN QUADRATURE, WHICH DELIVERS HIS ASK WITH NO SPECIAL CASE** (*"count almost the same…
unless there is a good amount of data that agrees"* — he described inverse-variance weighting from
first principles): a 12-rep set carries **86 %** of an 8-rep set's weight **on a machine**, where the
conversion is already the doubt, and **57 % on the key lift**, where σ_rep is nearly all that is left.
**The ladder said 53 % everywhere, blind to context.**
🔒 **A FAILING TEST INSISTED ON TWO FUNCTIONS**: folding reps into `sigmaFor()` broke *"the key lift
carries no CONVERSION uncertainty"*, still true of conversion and false of the function.
`readingSigma()` is separate for that reason. 🛑 **AND THE LOW-REP FILTER IS GONE** — it removed
longer sets *before* credibility was compared, so a 55×6 could hold a seat without ever outranking
anything. `repFactor` survives in the SEAT comparison only.
🔄 **Golden re-baselined again**: Core +6.5 %, Quads −0.6 %, Shoulders −0.7 %, rest under 0.3 %,
**both directions** (this re-weights rather than re-reads), every observation and contributor count
unchanged. 🔒 **Two of my own assertions were wrong and the suite said so** — a smoothness claim
measured on σ instead of on WEIGHT (σ jumps 37 % from 2 to 3 reps, which is 1.6 % → 2.2 % and means
nothing), and a claim that neither context matched the ladder when the key lift lands at 57 % vs 53 %.
🚩 **He also argued a high-rep set is good evidence of a MINIMUM** (right-skewed, since you cannot do
fewer reps than you did). **Mechanically right and deliberately NOT built** — correcting a recorded
set upward is the move the fatigue work refused in writing as *the only mechanism that can make a
number bigger than what was observed*. **His explicit decision, not a quiet build.**
...
🚨 **IT MUST RUN NO EARLIER THAN THE DAY COLLAPSE, AND THREE ATTEMPTS PROVED IT.** In
...
not by reading code** — the contributor count twice, the render suite once. 🔄 **Golden re-baselined**:
four muscles move (Back +10.2 %, Core +12.9 %, Glutes +17.2 %, Quads +11.4 %), **every move upward**,
**every observation and contributor count identical**. 🆕 `curveWeight` on the observation, because
re-reading a set at fewer reps needs the number the curve was fed and the alternative was a second
copy of D30.
...
assumption about what a COACH meant, and the alternative (assume failure) errs by making the bar
**heavier**. 🔒 **The transcribing agent found a bug in the module that sent it** — `clampReps()`
folded a prescribed **20 reps** to **15**, so the app quoted Nippard saying something he did not say;
the D5 gate belongs on the WEIGHT, never on the words. 🚨 **A second weak assertion was caught**: the
test that a prescription is priced lighter than failure **passed with the reserve zeroed**.
🛑 **10 of the 39 get reps and NO weight** — past D5 once the reserve is added.
✅ **All suites green — 5,680 assertions**, `data-layer` **2,413**, `render` **1,595**.
...
Explore; **Record shows only its workouts**. 🔒 **Both drawn by the same `systemBody()`.**
...
It reads one out of the training and **never writes what it derived**. 🔄 **It DELETED the folding
systems of 2026-09-16** — with one programme on screen a fold memory is a variable nothing reads.
🛑 **Adding or creating a programme does NOT make it current**; `#/system/<id>` has a button.
🚨 **FIVE OF ITS NEW ASSERTIONS WERE THE EMPTY ACCOUNT, whose empty states had NEVER been tested** —
without that guard the Workouts tab throws on a new account.
🛑 **AND A LIVE LINK TO PRE-BUILT SYSTEMS WAS ASSESSED AND REFUSED** — ✅ **the recommendation it
made instead was BUILT 2026-09-20**, above. 🔒 **A review agent found four false claims in the new
comments and all four were real; the first correction introduced a fifth** — now **§0.19**: grep for
what you are about to ASSERT. ⚠️ **The same review was WRONG about a layout claim**: an agent's
finding is a hypothesis with a line number attached. `docs/history.md`, 2026-09-19.

**2026-09-18, two sessions before it** — 🆕 **a planned set can carry a PERCENTAGE OF YOUR MAX**
(`js/set-targets.js`) and 🆕 **the label under the weight is the PLATES** (`js/plates.js`).
🚨 **The percentage is of your OWN best recorded set on that lift, never the cross-muscle estimate**;
body-weight and assisted lifts refused. 🔒 **The plate agent was told WHY greedy is optimal, measured
it, and the reason was FALSE** — the shipped pound inventory has **no 35s**; a justification in a
brief is a claim, not a premise. Open work 33 and 34; `docs/history.md` 2026-09-18.
...
**All twenty-two no-Chrome suites are green — 5,680 assertions**, `data-layer` **2,413**, `render`
**1,595**. ⚠️ **Three of those suites came from a PARALLEL AGENT in this same checkout, not from
...
🆕 **WHERE 2026-09-20 STOPPED.** Four passes, all finished, four commits pushed. **Nothing left
half-done.** 🚩 **THE TWO THINGS IT ADDED TO TIM'S LIST ARE ROWS 9 AND 10** — whether a high-rep set
should count as a FLOOR (his idea, and the one thing deliberately not built because it reverses a
written refusal), and **the rating still resting on three sets**, which is the biggest accuracy lever
left in the app. 🛑 **Neither is authorised.**
🔒 **AND THE METHOD LESSON OF THE DAY, because it cost three wrong implementations**: the rating code
has **two places that make the same weight-blind comparison** — `betterSameDay` per exercise-day and
`seatCredit` at the seat — and a rule enforced at one of them is not enforced. ⚠️ **Every failure was
caught by a COLUMN in the golden table (observation count, contributor count) or by the render suite,
never by reading the code.** The estimates looked plausible every time.
⚠️ **THE THING MOST LIKELY TO READ AS A BUG**: his copy of Nippard's PPL still has **no**
percentages, because a preset copy is a snapshot. **What is new is that the app now SAYS SO** — his
copy predates the stamps, so `#/workouts` shows *"the original of this programme changed"* and the
sheet lists it as **his to do**, offering no button. **That is correct**, not the feature failing;
an unstamped copy cannot tell his edits from the original's. *"Add another copy"* has the percentage.
🚩 **NOTHING FROM TODAY HAS BEEN DRIVEN IN CHROME**, and no preset has been changed for real — the
first genuine version bump is the first time the stamped path runs outside a test.
🚩 **AND 2026-09-19's TWO UNDECIDED CONSEQUENCES ARE UNCHANGED** — benchmark workouts in a separate
programme not reaching Record, and no way to look at a programme without switching to it. **Row 8**
carries both and the three answers.

🆕 **WHERE 2026-09-18 STOPPED.** All finished. 🚩 **TWO THINGS HE WAS TOLD AND HAS NOT ANSWERED**: a
targeted set is `prefilled`, so accepting the weight AND reps untouched drops it at save (row 7); and
🚨 **`targets` expresses ONE of the four percentages in his own programme** — the rest are a % of a
top set, of ANOTHER lift, or a drop, **both buildable and neither authorised**. 🆕 **Open work 32**:
the app never states its logging conventions where you log. **Words on screens.**

🆕 **WHERE 2026-09-17 AND -16 STOPPED.** Both finished. 🛑 **The two items Tim did NOT pick on -17
are not authorised** — the accessibility work (a deferral by name now; ⚠️ **do not offer it again**)
and pointing the audit at a friend's screens through the demo's deterministic uid. 🚩 **The thing to
expect a report about**: a friend's gender, age and friends list only appear after **their** app
republishes, and the calendar no longer draws the current month when the last recording is older
than it — both deliberate, both explained to him, both the sort of thing that reads as a bug.
...
## 🛑 THE TEN THINGS WAITING ON TIM — 9 and 10 added 2026-09-20
...
flags are rows 3 and 4 of **THE EIGHT THINGS WAITING ON TIM** above. 🔒 **The one thing that outlived
...
| **Render tests** | `npm i --no-save jsdom` then `node tests/render.test.mjs` — **1,534 assertions** (2026-09-16). 🆕 **Since 2026-09-16 it holds A FRIEND'S PROFILE** (their face and two tiles, the body row that never prints their weight, the best lifts that are read from their published grid and say "not ranked" for everything else, no goals section, the "View data" panel with its DOWN arrow and no Research, every old friend deep link, and **the back arrow landing on `#/me` from all five arrival routes** — Tim's override of Rule 8, unconditional since the depth version could not cover the panel he had just closed), **THE SYSTEM DISCLOSURES** (both screens, two memories, open by default, `#/system/<id>` surviving as a row), **A SYSTEM'S PLAN** (seven boxes and fourteen, Rest against nothing-planned, a dead workout id reading as unset, and the whole editor including the stepper that must not rebuild itself) and **THE CALENDAR'S COLLAPSED MONTHS AND MONTH CHART** (five months draws nothing and six draws it, the zero column in the right place, days-not-sessions on the axis, and *published* on a friend's). ⚠️ **41 mutations across five agents, each proved to have landed on code** — and the one that survived did so because the assertion matched the mutation's own marker comment, which is §0.14 in a new costume. ~~**1,360 assertions** (2026-09-15)~~, mounts every screen. 🆕 **Since 2026-09-15 it holds THE FATIGUE CAPTION'S WIRING** — the test the 2026-09-14 handover called the most valuable one missing. `rep-decrement.js` had 57 module assertions and nothing checked the multiplier ever reached the screen: set 1 reads *"maybe 9–15 to failure"*, set 2 *"maybe 6–11 to failure on this set (12 fresh)"*, set 3 falls further, and changing the load drops the note because a weight change is a fresh effort. 🚨 **Killing the decrement on its real path fails five assertions with the right symptom** — set 2 repeating set 1's sentence. ⚠️ **The first mutation tried was a §0.14 miss**: `setIndexMultiplier()` is not on that path at all, so it changed nothing and looked like a weak test. Also new: **the Data tab's dropped-sets caption with the D5 ceiling asserted**, and **the muscle panel's set-aside line** in full. 🆕 **Since 2026-09-12 it holds THE SET LOCK** (a suggestion never locks, a recorded set does, the padlock is a sibling of `.set-pick`, a drop locks with its set, per person, `locked` dropped at save), **THE FINGER-FOLLOWING DRAG** (rects stubbed at 56px; the dragged row is `translateY` of the travel, a passed row slides one height, the DOM is untouched until release, the grip's arrow keys move a row and keep focus), **THE RUNNER'S CAPTIONS** (per person — the guest's blank is the load-bearing one), **YEARS-FIRST CALENDARS** (all three doors; a tap on Months stamps `data-landed`; the two memories held at different values), and **THE RANKED BEST LIFTS** (level-first with the heavier lift second, the 1RM off the best set, one chip class, a real `<details>`). 🆕 **Since 2026-09-11 it holds THE PROFILE TAB'S FOUR SECTIONS** — the body row (what is there leads, what is missing is named, and it is a DOOR to `#/profile`), the goal row (**and no verdict word anywhere on the screen**), the best-lifts list (**days trained leads, the heavier one-day lift second**, measured over labelled estimate) and the calendar — plus 🔒 **that `#/me` holds no `input`, `textarea` or `select` at all**, which is the property the whole Data/Profile split rests on. 🆕 **Since 2026-09-09 it also holds THE RECORD PULL-UP** (the corner is a `down` arrow labelled Close, it lands on Home, and 🔒 **no parked screen is ever built in jsdom** — one would double every selector in the suite), **the PROFILE COUNTS** (two figures, one called Friends, no "?" on a private account, the public-account sentence ON the screen, and all three of `#/me/friends|followers|following` landing on a list titled Friends), and **the two wordiness splits** — the Goals safety claim asserted on the pane while the 2–10 % band and the lay-off refusal are read back out of the popover, and the Research tab's other-numbers paragraph likewise. 🆕 **A FRIEND'S MAP READ AGAINST THEM**, and the fixture is the assertion: **she is female while the reader is male**, because every earlier fixture published a male `defaultCompare` and so could not tell the right answer from the wrong one. It pins the caption (*women who lift*, and not *men* anywhere in it), that asking for men MOVES her level (the vacuity guard), that the caption follows the change — it used to be built once and never repainted — and the three pronouns: *Like them*, *Their body weight*, *Their age*, with *Own body weight* on the two-body screen. 🔒 **Plus the sheet's cut**: no `.preset-hint` anywhere, four axis dots, and the untrained-adult caveat read back **out of an opened popover** rather than off the pane. ⚠️ **And the STRUCTURE the laptop layout rests on** — the figure and the panel are siblings in one `.map-split`, and the shared-map host does NOT take `is-muscles`; jsdom has no layout, so this pins the shape and the browser measures the pixels. 🆕 **Since 2026-09-08 it holds THE SIMPLIFIED PROFILE MENU**: that a signed-in account is never shown a "Left on this device" card and no `/^Upload /` button (asserted on the SCREEN, because the card is what Tim objected to), and 🔒 **six caveat assertions that now CLICK the ? and read the popover back** — the demo card's two, the demo screen's "starts it over" plus a new one for Social being off, and the note's "goes straight to the person building it". ⚠️ **One could NOT be converted and was replaced honestly** — "the help text says what Edit is for" read a sentence that was deleted rather than moved, so it now asserts that the caption still names who can see the photo and that the sentence beside the Edit button is gone, not hidden. 🆕 **Since 2026-09-07 it holds LEAVING A WORKOUT OPEN**: that the runner's corner control keeps the session rather than ending it, that the bar names the workout and the exercise the WALK points at (asserted at step 3 of a superset, where `entries[index]` would read nothing), that yesterday's draft is not a live one, that the bin asks only when sets are recorded — and 🚨 **that starting a second workout no longer deletes the first**, which it did silently until that day. 🆕 **The SAVE SCREEN too**: 🚨 that **Finish saves nothing**, that the draft survives every path off it, and that a failed save says so **on the screen the user is looking at**. 🆕 **And the "?"** — a real button with an accessible name, Escape and outside-tap close it, one open at a time. 🔒 **The six assertions that guard the Volume and Goals caveats now OPEN the ? and read the words back**, which is stricter than the presence checks they replaced. **⚠️ THE AUGUST HALF IS COMPRESSED HERE — 2026-09-15, when this row alone reached 10 KB and pushed the file past its budget.** Every assertion it described is still in the suite; what follows is the part worth carrying, which is HOW they are written rather than what they cover. Full detail: `docs/history.md`, 2026-08-21 to -30. **Pictures, the swap shortlist, removing a person, the Research topics collapsed, the first-run prefill, where the steppers are, the friends screen with a hanging network, an offered workout writing nothing until Add, the cloud-full wording, the view/edit split** — plus the one assertion in this project that is a **budget rather than a presence check**, a muscle panel capped at 40 words, because nothing that checks a thing is THERE can catch prose piling back up. 🔒 **Four ways of writing an assertion, all learned the hard way and all still applied**: the first picture assertion is about **ABSENCE** (with no art bought nothing renders, which is the only thing that made it safe to ship ahead of the art); the remove control is **asserted with two guests on the bar, because with one it passes however the code is written**; back buttons and swap rows are asserted **by CLICKING**, the only version that caught five inert back buttons; and a nudge must repaint the row **in place**, asserted by holding the row NODE across the change, because a rebuild would destroy the input under the user's finger |
